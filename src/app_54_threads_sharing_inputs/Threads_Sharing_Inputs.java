...
public class Threads_Sharing_Inputs {
    ...
    public static void main(String[] args) {
        ...
        Thread thread_1 = new Thread(() -> {
            new ArrayList<Integer>(Arrays.asList(new Integer[1000])).stream().forEach(i -> counter.incrementCounter());
        });
        ...
        Thread sync_thread_1 = new Thread(() -> {
            new ArrayList<Integer>(Arrays.asList(new Integer[1000])).stream().forEach(i -> sync_counter.incrementCounter());
        });
        Thread sync_thread_2 = new Thread(() -> {
            new ArrayList<Integer>(Arrays.asList(new Integer[1000])).stream().forEach(i -> sync_counter.incrementCounter());
        });
        Thread sync_thread_3 = new Thread(() -> {
            new ArrayList<Integer>(Arrays.asList(new Integer[1000])).stream().forEach(i -> sync_counter.incrementCounter());
        });
        Thread sync_thread_4 = new Thread(() -> {
            new ArrayList<Integer>(Arrays.asList(new Integer[1000])).stream().forEach(i -> sync_counter.incrementCounter());
        });

        Thread list_thread_1 = new Thread(() -> {
            new ArrayList<Integer>(Arrays.asList(new Integer[1000])).stream().forEach(i -> list.add(list.size() + 1));
        });
        Thread list_thread_2 = new Thread(() -> {
            new ArrayList<Integer>(Arrays.asList(new Integer[1000])).stream().forEach(i -> list.add(list.size() + 1));
        });
        Thread list_thread_3 = new Thread(() -> {
            new ArrayList<Integer>(Arrays.asList(new Integer[1000])).stream().forEach(i -> list.add(list.size() + 1));
        });
        Thread list_thread_4 = new Thread(() -> {
            new ArrayList<Integer>(Arrays.asList(new Integer[1000])).stream().forEach(i -> list.add(list.size() + 1));
        });

        Thread sync_list1_thread_1 = new Thread(() -> {
            new ArrayList<Integer>(Arrays.asList(new Integer[1000])).stream().forEach(i -> sync_list1.incrementList());
        });
        Thread sync_list1_thread_2 = new Thread(() -> {
            new ArrayList<Integer>(Arrays.asList(new Integer[1000])).stream().forEach(i -> sync_list1.incrementList());
        });
        Thread sync_list1_thread_3 = new Thread(() -> {
            new ArrayList<Integer>(Arrays.asList(new Integer[1000])).stream().forEach(i -> sync_list1.incrementList());
        });
        Thread sync_list1_thread_4 = new Thread(() -> {
            new ArrayList<Integer>(Arrays.asList(new Integer[1000])).stream().forEach(i -> sync_list1.incrementList());
        });

        Thread sync_list2_thread_1 = new Thread(() -> {
            new ArrayList<Integer>(Arrays.asList(new Integer[1000])).stream().forEach(i -> sync_list2.add(list.size() + 1));
        });
        Thread sync_list2_thread_2 = new Thread(() -> {
            new ArrayList<Integer>(Arrays.asList(new Integer[1000])).stream().forEach(i -> sync_list2.add(list.size() + 1));
        });
        Thread sync_list2_thread_3 = new Thread(() -> {
            new ArrayList<Integer>(Arrays.asList(new Integer[1000])).stream().forEach(i -> sync_list2.add(list.size() + 1));
        });
        Thread sync_list2_thread_4 = new Thread(() -> {
            new ArrayList<Integer>(Arrays.asList(new Integer[1000])).stream().forEach(i -> sync_list2.add(list.size() + 1));
        });

        Thread primitive_thread1 = new Thread(() -> {
            while (!check) System.out.println("check = false");
        });
        Thread primitive_thread2 = new Thread(() -> {
            check = true;
